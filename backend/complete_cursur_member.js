...
async function completeCursurMember() {
  const memberId = 18; // The latest Cursur member ID
  
  try {
    console.log(`🚀 Completing data for Cursur member ID: ${memberId}...`);
    
    // 1. Add to members_registration_details table
    await dbQuery(`
    ...
    // 2. Add to addresses table (Registered Office Address)
    await dbQuery(`
    ...
    // 6. Add to certification_details table
    await dbQuery(`
    ...
    console.error('❌ Error completing member:', error);
  } finally {
    db.end();
  }
}
